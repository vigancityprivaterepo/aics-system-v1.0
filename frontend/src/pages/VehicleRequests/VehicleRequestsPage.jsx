...
import { AlertTriangle, Ambulance, CheckCircle2, Download, Eye, Pencil, Plus, Search, Trash2, X, XCircle } from 'lucide-react'
...
const reviewStatus = (item) => item.status === 'approved' ? 'Approved by Administration' : item.status === 'disapproved' ? 'Disapproved by Administration' : 'Pending Administrative Approval'
const statusClass = (item) => item.status === 'disapproved'
  ...
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-3 sm:p-6">
    <div className="mx-auto w-full max-w-5xl bg-slate-50 shadow-2xl">
      ...
      <form onSubmit={submit} className="space-y-6 p-5">
        ...
        {initial?.status === 'disapproved' && <div className="border-l-4 border-red-500 bg-red-50 px-4 py-3 text-sm text-red-900">This request was disapproved by Administration. Update the details and save to resubmit it for approval.</div>}
        ...
        {initial?.choReviewedAt && <div className={`border-l-4 px-4 py-3 text-sm ${initial.status === 'disapproved' ? 'border-red-500 bg-red-50 text-red-900' : 'border-emerald-600 bg-emerald-50 text-emerald-900'}`}>CHO result: <strong>{reviewStatus(initial)}</strong> by {initial.choReviewedByName || 'City Health Office'}.</div>}
...
export default function VehicleRequestsPage() {
  ...
  const disapprove = async (item) => { const reason = window.prompt(`Disapprove ${item.requestNumber}?\n\nOptionally provide a reason for the City Health Office (leave blank to skip):`); if (reason === null) return; try { await api.patch(`/vehicle-requests/${item.id}/disapprove`, { reason: reason.trim() || undefined }); await load(); toast.success('Vehicle request disapproved.') } catch (error) { toast.error(error.response?.data?.message || 'Failed to disapprove vehicle request.') } }
  ...
  const pendingCount = useMemo(() => requests.filter((r) => r.status !== 'approved' && r.status !== 'processed' && r.status !== 'disapproved').length, [requests])
