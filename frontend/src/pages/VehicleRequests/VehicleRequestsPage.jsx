...
import { Ambulance, CheckCircle2, Download, Pencil, Plus, Search, Trash2, X } from 'lucide-react'
...
const EMPTY = {
  ...
  driverPerDiem: 0, rentalFees: 0, tollFees: 0, otherFees: 0, fuelExpenses: 0, remarks: '', status: 'pending_cho_review',
}
...
const reviewStatus = (item) => {
  if (item.status === 'cho_reviewed' || item.status === 'processed') {
    return item.availability === 'available' ? 'Ambulance Available' : item.availability === 'unavailable' ? 'Ambulance Not Available' : 'CHO Reviewed'
  }
  return 'Pending CHO Review'
}
...
function RequestForm({ initial, options, onClose, onSaved }) {
  ...
  const changeOffice = (value) => { setOfficeChoice(value); set('office', value === '__other__' ? customOffice : value) }
  const changeCustomOffice = (value) => { setCustomOffice(value); set('office', value) }
    ...
    <div className="mx-auto w-full max-w-5xl bg-slate-50 shadow-2xl">
      ...
      <form onSubmit={submit} className="space-y-6 p-5">
        <section><h3 className="mb-3 border-b border-slate-200 pb-2 text-sm font-semibold text-slate-900">Request Information</h3><div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div><label className="portal-label">Office / Organization *</label><select required className="portal-input" value={officeChoice} onChange={(event) => changeOffice(event.target.value)}><option value="">Select office or organization</option>{officeChoices.map((office) => <option key={office} value={office}>{office}</option>)}<option value="__other__">Other Office / Organization</option></select>{officeChoice === '__other__' && <input required className="portal-input mt-2" value={customOffice} onChange={(event) => changeCustomOffice(event.target.value)} placeholder="Specify office or organization" />}</div>
        ...
        <div className="sticky bottom-0 flex justify-end gap-3 border-t border-slate-200 bg-slate-50 py-4"><button type="button" onClick={onClose} className="portal-button-secondary">Cancel</button><button disabled={saving} className="portal-button-primary">{saving ? 'Saving...' : initial?.id ? 'Save Changes' : 'Submit to CHO'}</button></div>
      </form>
    </div>
  </div>
}

function ChoReviewForm({ request, onClose, onSaved }) {
  const [form, setForm] = useState({
    availability: request.availability === 'unavailable' ? 'unavailable' : 'available',
    vehicleModel: request.vehicleModel || '', unavailableReason: request.unavailableReason || '', plateNumber: request.plateNumber || '',
    alternativeVehicle: request.alternativeVehicle || '', alternativeModel: request.alternativeModel || '', alternativePlate: request.alternativePlate || '', remarks: request.remarks || '',
  })
  const [saving, setSaving] = useState(false)
  const set = (field, value) => setForm((current) => ({ ...current, [field]: value }))
  const submit = async (event) => {
    ...
    try {
      const { data } = await api.patch(`/vehicle-requests/${request.id}/cho-review`, form)
      toast.success(`${request.requestNumber} reviewed by City Health Office.`); onSaved(data)
    } catch (error) { toast.error(error.response?.data?.message || 'Failed to save CHO review.') }
    finally { setSaving(false) }
  }
  return <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-3 sm:p-6"><div className="mx-auto w-full max-w-2xl bg-white shadow-2xl">
    <div className="flex items-center justify-between bg-emerald-800 px-5 py-4 text-white"><div><h2 className="text-lg font-semibold">CHO Review</h2><p className="text-xs text-emerald-100">{request.requestNumber} · {request.requestedBy}</p></div><button type="button" onClick={onClose} className="flex h-9 w-9 items-center justify-center" title="Close"><X className="h-5 w-5" /></button></div>
    <form onSubmit={submit} className="space-y-5 p-5">
      <div className="grid grid-cols-1 gap-3 border-b border-slate-200 pb-4 text-sm sm:grid-cols-2"><div><span className="text-slate-500">Destination</span><p className="font-medium text-slate-900">{request.destination}</p></div><div><span className="text-slate-500">Travel</span><p className="font-medium text-slate-900">{formatTravelDateTime(request.departureDate, request.departureTime)}</p></div></div>
      <div><label className="portal-label">Ambulance Availability *</label><select className="portal-input" value={form.availability} onChange={(e) => set('availability', e.target.value)}><option value="available">Available</option><option value="unavailable">Not Available</option></select></div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div><label className="portal-label">Make / Model</label><input className="portal-input" value={form.vehicleModel} onChange={(e) => set('vehicleModel', e.target.value)} /></div><div><label className="portal-label">Plate Number</label><input className="portal-input" value={form.plateNumber} onChange={(e) => set('plateNumber', e.target.value)} /></div></div>
      {form.availability === 'unavailable' && <><div><label className="portal-label">Reason for Unavailability *</label><textarea required className="portal-input min-h-20" value={form.unavailableReason} onChange={(e) => set('unavailableReason', e.target.value)} /></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-3"><div><label className="portal-label">Alternative Vehicle</label><input className="portal-input" value={form.alternativeVehicle} onChange={(e) => set('alternativeVehicle', e.target.value)} /></div><div><label className="portal-label">Alternative Model</label><input className="portal-input" value={form.alternativeModel} onChange={(e) => set('alternativeModel', e.target.value)} /></div><div><label className="portal-label">Alternative Plate</label><input className="portal-input" value={form.alternativePlate} onChange={(e) => set('alternativePlate', e.target.value)} /></div></div></>}
      <div><label className="portal-label">CHO Remarks</label><textarea className="portal-input min-h-20" value={form.remarks} onChange={(e) => set('remarks', e.target.value)} /></div>
      <div className="flex justify-end gap-3 border-t border-slate-200 pt-4"><button type="button" onClick={onClose} className="portal-button-secondary">Cancel</button><button disabled={saving} className="portal-button-primary"><CheckCircle2 className="h-4 w-4" />{saving ? 'Saving...' : 'Confirm CHO Review'}</button></div>
    </form>
  </div></div>
}
...
export default function VehicleRequestsPage() {
  ...
  const [requests, setRequests] = useState([]), [options, setOptions] = useState({}), [search, setSearch] = useState(''), [status, setStatus] = useState(''), [editing, setEditing] = useState(undefined), [reviewing, setReviewing] = useState(null), [loading, setLoading] = useState(true)
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="portal-kicker">Transport Services</p><h1 className="portal-page-title">{isCho ? 'Ambulance Requests' : 'Vehicle Requests'}</h1><p className="portal-page-subtitle">{isCho ? 'Review ambulance availability requested by case makers.' : 'Submit requests and track City Health Office ambulance availability.'}</p></div>{!isCho && <button className="portal-button-primary" onClick={() => setEditing(null)}><Plus className="h-4 w-4" /> New Request</button>}</div>
    <div className="flex flex-col gap-3 border-y border-slate-200 bg-white px-4 py-3 sm:flex-row"><form className="flex flex-1 gap-2" onSubmit={(e)=>{e.preventDefault();load()}}><div className="relative flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400"/><input className="portal-input pl-9" value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search control number, requester, office, destination" /></div><button className="portal-button-secondary">Search</button></form><select className="portal-input sm:w-56" value={status} onChange={(e)=>setStatus(e.target.value)}><option value="">All review statuses</option><option value="pending_cho_review">Pending CHO Review</option><option value="cho_reviewed">CHO Reviewed</option></select></div>
    <div className="overflow-hidden border border-slate-200 bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr>{['Control No.','Request Date','Requester / Office','Vehicle','Destination','Travel Date','CHO Status','Reviewed By',''].map(h=><th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody>{loading?<tr><td colSpan="9" className="px-4 py-10 text-center text-slate-400">Loading requests...</td></tr>:requests.length===0?<tr><td colSpan="9" className="px-4 py-10 text-center text-slate-400">No vehicle requests found.</td></tr>:requests.map(item=><tr key={item.id} className="border-t border-slate-100"><td className="px-4 py-3 font-mono font-semibold text-emerald-800">{item.requestNumber}</td><td className="px-4 py-3">{dateOnly(item.requestDate)}</td><td className="px-4 py-3"><p className="font-medium text-slate-900">{item.requestedBy}</p><p className="text-xs text-slate-500">{item.office}</p></td><td className="px-4 py-3">{vehicleLabel(item.vehicleType,item.otherVehicle)}</td><td className="max-w-56 px-4 py-3">{item.destination}</td><td className="px-4 py-3">{formatTravelDateTime(item.departureDate, item.departureTime)}</td><td className="px-4 py-3"><span className={`inline-flex px-2 py-1 text-xs font-medium ${statusClass(item)}`}>{reviewStatus(item)}</span></td><td className="px-4 py-3"><p>{item.choReviewedByName || '—'}</p>{item.choReviewedAt && <p className="text-xs text-slate-500">{dateOnly(item.choReviewedAt)}</p>}</td><td className="px-4 py-3"><div className="flex gap-1"><button title="Download DOCX" onClick={()=>download(item)} className="flex h-9 w-9 items-center justify-center text-emerald-700 hover:bg-emerald-50"><Download className="h-4 w-4"/></button>{isCho ? <button title="Review ambulance request" onClick={()=>setReviewing(item)} className="flex h-9 w-9 items-center justify-center text-emerald-700 hover:bg-emerald-50"><CheckCircle2 className="h-4 w-4"/></button> : <><button title="Edit request" onClick={()=>setEditing(item)} className="flex h-9 w-9 items-center justify-center text-slate-600 hover:bg-slate-100"><Pencil className="h-4 w-4"/></button><button title="Delete request" onClick={()=>remove(item)} className="flex h-9 w-9 items-center justify-center text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4"/></button></>}</div></td></tr>)}</tbody></table></div></div>
    ...
    {reviewing && <ChoReviewForm request={reviewing} onClose={()=>setReviewing(null)} onSaved={async()=>{setReviewing(null);await load()}} />}
  </div>
}