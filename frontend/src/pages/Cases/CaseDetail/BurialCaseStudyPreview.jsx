...
const DOCS = [
  { key: 'case-study',       label: 'Burial Case Study', endpoint: (id) => `/cases/${id}/report/docx`,    downloadLabel: 'case-study' },
...
export default function BurialCaseStudyPreview({ caseData }) {
  const [docIndex, setDocIndex]   = useState(0)
  const [docState, setDocState]   = useState({ 'case-study': { loading: true, error: false } }) // { [key]: { loading, error } }
  const abCache   = useRef({})                   // { [key]: ArrayBuffer }
  ...
  const doRender = useCallback(async (key, arrayBuffer) => {
    ...
    try {
      // null styleContainer → docx-preview injects <style> tags into containerRef itself
