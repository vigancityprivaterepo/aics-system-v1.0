...
const ImgManager = require("./imgManager.cjs");

function isNaNValue(number) {
...
function getInnerPptx({ part, left, right, postparsed }) {
  const xmlString = postparsed.slice(left + 1, right).reduce((concat, item) => concat + item.value, "");
...
class ImageModule {
  ...
  parse(placeHolderContent) {
    ...
    if (placeHolderContent.substring(0, 2) === "%%") {
      return { type: "placeholder", value: placeHolderContent.substr(2), module: moduleName, centered: true };
    }
    if (placeHolderContent.substring(0, 1) === "%") {
      return { type: "placeholder", value: placeHolderContent.substr(1), module: moduleName, centered: false };
    }
    return null;
  }
  ...
  resolve(part, options) {
    ...
    return Promise.resolve(this.options.getImage(value, part.value))
      .then((imgBuffer) => {
        const rId = imgManager.addImageRels(this.getNextImageName(), imgBuffer);
        return Promise.resolve(this.options.getSize(imgBuffer, value, part.value)).then((sizePixel) => ({
          rId,
          sizePixel,
        }));
      });
  ...
  getRenderedPart(part, rId, sizePixel) {
    ...
    const newText = this.fileType === "pptx"
      ? this.getRenderedPartPptx(part, rId, size, centered)
      : this.getRenderedPartDocx(rId, size, centered);
