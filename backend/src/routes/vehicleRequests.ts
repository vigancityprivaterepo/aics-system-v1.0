...
const STATUS = ['draft', 'processed', 'pending_cho_review', 'cho_reviewed'] as const
...
const requestSchema = z.object({
  ...
  status: z.enum(STATUS).default('pending_cho_review'),
...
router.get('/options', asyncHandler(async (req, res) => {
  const rows = await prisma.vehicleRequest.findMany({
    where: isCho(req) ? { vehicleType: 'ambulance' } : undefined,
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const requests = await prisma.vehicleRequest.findMany({
    where: {
      ...(isCho(req) ? { vehicleType: 'ambulance' } : {}),
      ...(status ? { status } : {}),
      ...(search ? { OR: [
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if (isCho(req)) throw new HttpError(403, 'City Health Office reviews ambulance requests but cannot create them.')
  ...
  const created = await prisma.vehicleRequest.create({
    data: { ...body, requestNumber, availability: 'pending', status: 'pending_cho_review', createdById: req.user.id },
  })
  await logAdminAudit(prisma, { actorId: req.user.id, action: 'vehicle_request.create', targetType: 'vehicle_request', targetId: created.id, summary: `Created vehicle request ${requestNumber} for CHO review` })
...
router.put('/:id', asyncHandler(async (req, res) => {
  if (isCho(req)) throw new HttpError(403, 'Use CHO Review to record ambulance availability.')
  const body = requestSchema.parse(req.body)
  const current = await prisma.vehicleRequest.findUnique({ where: { id: String(req.params.id) } })
  if (!current) throw new HttpError(404, 'Vehicle request not found.')
...
router.delete('/:id', asyncHandler(async (req, res) => {
  if (isCho(req)) throw new HttpError(403, 'City Health Office cannot delete vehicle requests.')
